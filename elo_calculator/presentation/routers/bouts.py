...
from elo_calculator.application.bout_service import BoutService
...
bouts_router = APIRouter(prefix='/bouts', tags=['Bouts'])


@bouts_router.get('/', response_model=MainResponse[list[BoutResponse]])
...
@bouts_router.get('/{bout_id}', response_model=MainResponse[BoutResponse])
async def get_bout(
...
@bouts_router.post('/', response_model=MainResponse[BoutResponse])
...
@bouts_router.get('/{bout_id}/details', response_model=MainResponse[BoutDetailsResponse])
