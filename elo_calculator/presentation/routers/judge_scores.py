...
async def list_by_bout(
    ...
    return get_ok(JudgeScoreResponse.from_entity_list(scores))
...
async def get_judge_score(
    ...
    if score is None:
        return get_not_found(message=f'JudgeScore bout_id:{bout_id} fighter_id:{fighter_id} not found')
    return get_ok(JudgeScoreResponse.from_entity(score))
