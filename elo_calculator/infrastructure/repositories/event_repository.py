...
class EventRepository(BaseRepository[Event]):
    ...
    async def get_by_event_link(self, event_link: str) -> Event | None:
        """Get a single event by its canonical event_link."""
        cmd = select(self.table).where(self.table.c.event_link == event_link)
        result = await self.connection.execute(cmd)
        row = result.first()
        return self._map_row_to_model(row._asdict()) if row else None
